...
static struct snd_soc_dai_link msm8952_quin_dai_link[] = {
	/*{
      ...
      //bug 291165,yangbo.wt,20170911,add, audio bringup for modify mi2s tx to feedback                                  
        {
		.name = LPASS_BE_QUIN_MI2S_TX,
		.stream_name = "Quinary MI2S Capture",
		.cpu_dai_name = "msm-dai-q6-mi2s.5",
	...
	for (i = 0; i < card->num_links; i++) {
		...
		if (dai_link[i].platform_name &&
			...
			dai_link[i].platform_name = NULL;
                    printk("dai_link[%d].platform_of_node=%s,.platform_name=%s\n",i,dai_link[i].platform_of_node->name,dai_link[i].platform_name); 
...
cpu_dai:
		...
		if (dai_link[i].cpu_dai_name && !dai_link[i].cpu_of_node) {
			...
			dai_link[i].cpu_dai_name = NULL;
                     printk("dai_link[%d].cpu_of_node=%s,.cpu_dai_namee=%s\n",i,dai_link[i].cpu_of_node->name,dai_link[i].cpu_dai_name);
...
codec_dai:
		...
		if (dai_link[i].codec_name && !dai_link[i].codec_of_node) {
			...
			dai_link[i].codec_name = NULL;
                       printk("dai_link[%d].codec_of_node=%s,.codec_name=%s\n",i,dai_link[i].codec_of_node->name,dai_link[i].codec_name);  
